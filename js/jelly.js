...
class Jelly {
    constructor(x, y, color, radius) {
        ...
        for (var i = 0; i < 50; i++) {
            var alpha = 2 * Math.PI * i / 50;
            this
                .dots
                .push(new Dot(this.x + Math.cos(alpha) * radius, this.y + Math.sin(alpha) * radius))
        }
    }
    dots = [];
    ...
    Simulate(m) {
        for (var i = 0; i < this.dots.length; i++) {
            ...
            //surface tension !!!HEAVY!!!
            var dp = (i > 0
                ? this.dots[i - 1]
                : this.dots[this.dots.length - 1]);
            var dn = (i < this.dots.length - 1
                ? this.dots[i + 1]
                : this.dots[0]);
            ...
            //this.dots[i].vx += ()*this.tensionFactor; move the dot
    ...
    Draw(context) {
        ...
        context.beginPath();
        for (var i = 0; i <= this.dots.length; i++) {
            var d1 = this.dots[i >= this.dots.length
                    ? i - this.dots.length
                    : i];
            var d2 = this.dots[i + 1 >= this.dots.length
                    ? i + 1 - this.dots.length
                    : i + 1];
